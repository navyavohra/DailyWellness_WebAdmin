...
declare var $: any;
@Component({
  selector: 'app-relax-media',
  templateUrl: './relax-media.component.html',
  styleUrls: ['./relax-media.component.scss']
})
export class RelaxMediaComponent implements OnInit {
  allUsers: any;
  resultdata: any;
  file: any;
  ...
  async getVideos() {
    this.allUsers = await this.services.getAllrelaxVideos();
...
Close() {
  ...
  ConfirmDelete() {
    this.services.getrelaxVideos(this.deleteId).subscribe(
      (data: any) => {
    ...
    this.services.deleterelaxVideos(this.deleteId);
  ...
  getVideosbyId(id: any) {
    this.updateId = id;
    this.services.getrelaxVideos(id).subscribe(
      (data: any) => {
  ...
  async UploadFile(type: any) {
    if(this.file && this.thumbfile){
      this.countdata = await this.services.count('Relax-media');
      ...
      const storageRef = this.storage.ref("Relax-media/"+this.countdata[0].count + "-" +filePath);
      const uploadTask = this.storage.upload("Relax-media/"+this.countdata[0].count + "-" +filePath, this.file);
      ...
      uploadTask.snapshotChanges().pipe(finalize(() => {
        storageRef.getDownloadURL().subscribe(async downloadURL => {
          this.fileurl = downloadURL;
            ...
            const thumbstorageRef = this.storage.ref("Relax-media/" + this.countdata[0].count + "-" +thumbfilePath);
            const thumbuploadTask = this.storage.upload("Relax-media/" + this.countdata[0].count + "-" +thumbfilePath, this.thumbfile);
            ...
            thumbuploadTask.snapshotChanges().pipe(finalize(() => {
              thumbstorageRef.getDownloadURL().subscribe(downloadURL => {
                ...
                if (type == 'add') {
                  this.services.addrelaxVideos(this.type, this.thumbfileurl, this.fileurl);
                  $('#AddModal').modal('hide');
                  this.successMessage = "Media added successfully.";
                  $('#SuccessModal').modal('show');
                }
                if (type == 'update') {
                  this.services.updaterelaxVideos(this.updateId,this.type, this.thumbfileurl, this.fileurl);
